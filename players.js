// ============================================================
//  MC MOBILE — players.js (ВСЕ ИГРОКИ)
// ============================================================
...
function getPlayerRating(id) {
    return PLAYER_DB[id] ? PLAYER_DB[id].rating : 50;
}

function getPlayerSpeed(rating) {
    // 50 -> 1.0, 99 -> 5.0
    return 1.0 + (rating - 50) * 0.0816;
}

function getPlayerPower(rating) {
    return 2.8 + (rating - 50) * 0.057;
}

// ---------- ПОСТРОЕНИЕ МЕНЮ ИГРОКОВ ----------
function buildPlayerMenu() {
    ...
    ids.forEach(id => {
        ...
        card.addEventListener('click', function() {
            ...
            // Сохраняем выбранного игрока в глобальную переменную
...
function initPlayers() {
    ...
    if (mode === '2v2') {
        teamA_ids = [0, 1];      // Хабиб, Азиз
        teamB_ids = [2, 4];      // Шамиль Рб, Салаудин
    } else {
        teamA_ids = [0, 1, 5];   // Хабиб, Азиз, Абдул
        teamB_ids = [2, 3, 4];   // Шамиль Рб, Шамиль Jr, Салаудин
    }
    ...
    allIds.forEach(id => {
        ...
        const data = {
            ...p,
            rating: rating,
            isPlayer: p.id === selectedId,
            team: isA ? 'A' : 'B',
            isGoalkeeper: false,
            speed: getPlayerSpeed(rating) * 1.8,
            power: getPlayerPower(rating) * 1.2,
            x: 0, y: 0,
            targetX: 0, targetY: 0,
            r: 18,
            stamina: 1.0,
    ...
    teamA_data.forEach((p, i) => {
        p.x = 140;
        p.y = 180 + spacingA * (i + 1);
        p.targetX = p.x + (Math.random() - 0.5) * 100;
        p.targetY = p.y + (Math.random() - 0.5) * 100;
    });
    teamB_data.forEach((p, i) => {
        p.x = 460;
        p.y = 180 + spacingB * (i + 1);
        p.targetX = p.x + (Math.random() - 0.5) * 100;
        p.targetY = p.y + (Math.random() - 0.5) * 100;
    });

    // Вратари
    if (teamA_data.length > 0) {
        const gkA = teamA_data[0];
        gkA.isGoalkeeper = true;
        gkA.x = 40;
        gkA.y = 400;
    }
    if (teamB_data.length > 0) {
        const gkB = teamB_data[0];
        gkB.isGoalkeeper = true;
        gkB.x = 560;
        gkB.y = 400;
    }

    // Если игрок вратарь — делаем полевым
    ...
    console.log(`✅ Игроки инициализированы (${window.allPlayers.length} чел.)`);
